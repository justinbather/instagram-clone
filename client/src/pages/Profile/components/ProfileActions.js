...
export const ProfileActions = (props) => {
    ...
    if (props.isOwner) { //* Display editing buttons 
        return(

            <div className="flex flex-row w-full justify-center items-center gap-1">   
                <button onClick={props.toggleEditView} className="h-6 w-1/2 bg-zinc-800 rounded-md text-center font-bold text-white font-inter text-sm">Edit profile</button>
                <button onClick={handleFollow} className="h-6 w-1/2 bg-zinc-800 rounded-md text-center font-bold text-white font-inter text-sm">Share profile</button>
            </div>
        )
    } else { //* User profile is not the logged in users. Don't display editing buttons
        return(
